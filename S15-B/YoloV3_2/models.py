...
def create_modules(module_defs, img_size):
    ...
    output_filters = [256]  # input channels # change_naman
...
#             print(mdef['layers'])
            layers = mdef['layers'] # (-1, 61)
            filters = sum([output_filters[l + 1 if l > 0 else l] for l in layers]) # [OutF[62], OutF[-1]]
            routs.extend([i + l if l < 0 else l for l in layers]) # [i-1, 61]
