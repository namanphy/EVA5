...
class LRFinder(object):
    ...
    def range_test(
        ...
        for iteration in tqdm(range(num_iter)):
            ...
            metric_value = loss if self.metric == 'loss' else accuracy
    ...
    def _validate(self, val_iter, non_blocking_transfer=True):
        ...
        accuracy = 100 * correct / len(val_iter.dataset)
    ...
    def plot(
        ...
        lrs = self.history["lr"]
        metric = self.history["metric"]
        if skip_end == 0:
            lrs = lrs[skip_start:]
            metric = metric[skip_start:]
        else:
            lrs = lrs[skip_start:-skip_end]
            metric = metric[skip_start:-skip_end]
        ...
        if suggest_lr:
            ...
            min_grad_idx = None
            try:
                min_grad_idx = (np.gradient(np.array(metric))).argmin()
            except ValueError:
                print(
        ...
        ax.set_ylabel(self.metric.title())
